...
datas  = [data2i0, data2i1, data2i2, data4i3, data6i4]
datas += [data2a0, data2a1, data2a2, data4a3, data6a4]
...
ax[cases[0]].set_ylabel("Filtered Lines",fontsize=FONTSIZE)
...
for case in cases:    
    ...
    if case == "ideal":
        ...
        _title = "+ ambipolar diffusion"#"+ non-ideal MHD"
        m = "a" 

    ax[case].set_title(_title,fontsize=FONTSIZE)
    ...
    for _, data_j in enumerate(datas): 
        ...
        surf    = data_j["surf"]
        radius  = data_j['rloc']
        _label  = f"$n_c > $" + f"$10^{_id[0]}$\n"
        _label  += f"$r_c < $" + f"${np.round(radius,3)}$"
        
        ax[case].plot(_time, 1.0 - surf, ".-", linewidth=2, label=_label)
        ax[case].legend(fontsize=FONTSIZE - 8, frameon=False, loc = "upper left")

plt.savefig(f'./paper/filteredout.png', dpi=150, bbox_inches='tight')
plt.close(fig)
...
if False:
    ...
    for _, data_j in enumerate(datas): 
        ...
        for j in range(total_snaps-1,0,-1):
            ...
            if False:
                ...
                _slice = np.logical_and((z < radius/10), (z > -radius/10))
            ...
            print(f'$t$ = {round(_time, 5)}  Myrs\nPoints: ' + f'$n_c > 10^{_id[0]}$ ' + r'$\rm{cm}^{-3}$')
    ...
    axd["left"].set_xlabel("$t$ [Myrs]",fontsize=FONTSIZE)
    ...
    axd["middle"].plot(data2i1['t'], ratio0 - ratio2, '--', color=COLORS[1])
    axd["middle"].set_xlabel("$t$ [Myrs]",fontsize=FONTSIZE)
    ...
    axd["right"].plot(data2i2['t'], ratio0 - ratio2, '--', color=COLORS[1])
    axd["right"].set_xlabel("$t$ [Myrs]",fontsize=FONTSIZE)
    
    axd["right"].tick_params(labelleft=False)
    axd["right"].grid(True, which='both', alpha=GRID_ALPHA)
    fig.suptitle(title)
    plt.savefig( "./series/" + 'ratiosi012.png', dpi=300)
    plt.close(fig)

    ratio0 = np.array([np.mean(nlos0/ncrs) for (nlos0,ncrs) in zip(data2a0["Nlos0"],data2a0["Ncrs"])]) 
    ratio1 = np.array([np.median(nlos1/ncrs) for (nlos1,ncrs) in zip(data2a0["Nlos0"],data2a0["Ncrs"])])
    ratio2 = np.array([np.std(nlos0/ncrs) for (nlos0,ncrs) in zip(data2a0["Nlos0"],data2a0["Ncrs"])])

    texto = r"$r^{non-ideal}_{cloud} = 0.1 \ \rm{pc}$ "
    #axd["left"].plot(data2i0['t'], np.ones_like(data2i0['t']), '--', color="black")
    axd["left"].text(data2a0['t'][0], 1.70, texto, fontsize=12, color="black", ha="left", va="bottom")
    axd["left"].plot(data2a0['t'], ratio0, 'o-', label=r"Mean $N_{los}/N_{crs}$")
    axd["left"].plot(data2a0['t'], ratio1, '.-', label=r"Median $N_{los}/N_{crs}$")
    axd["left"].plot(data2a0['t'], ratio0 + ratio2, '--', color=COLORS[1], label="$\sigma$-band")
    axd["left"].plot(data2a0['t'], ratio0 - ratio2, '--', color=COLORS[1])
    axd["left"].set_ylabel(r"$N_{los}/N_{crs}$",fontsize=FONTSIZE)
    axd["left"].set_xlabel("$t$ [Myrs]",fontsize=FONTSIZE)
    axd["left"].grid(True, which='both', alpha=GRID_ALPHA)
    axd["left"].legend(frameon = False)

    ratio0 = np.array([np.mean(nlos0/ncrs) for (nlos0,ncrs) in zip(data2a1["Nlos0"],data2a1["Ncrs"])]) 
    ratio1 = np.array([np.median(nlos1/ncrs) for (nlos1,ncrs) in zip(data2a1["Nlos0"],data2a1["Ncrs"])])
    ratio2 = np.array([np.std(nlos0/ncrs) for (nlos0,ncrs) in zip(data2a1["Nlos0"],data2a1["Ncrs"])])
    print(ratio0.shape)
    texto = r"$r^{non-ideal}_{cloud} = 0.2 \ \rm{pc}$"
    axd["middle"].text(data2a1['t'][0], 1.70, texto, fontsize=12, color="black", ha="left", va="bottom")
    #axd["middle"].plot(data2i1['t'], np.ones_like(data2i1['t']), '--', color="black")
    axd["middle"].plot(data2a1['t'], ratio0, 'o-', label="Mean ratio")
    axd["middle"].plot(data2a1['t'], ratio1, '.-', label="Median ratio")
    axd["middle"].plot(data2a1['t'], ratio0 + ratio2, '--', color=COLORS[1])
    axd["middle"].plot(data2a1['t'], ratio0 - ratio2, '--', color=COLORS[1])
    axd["middle"].set_xlabel("$t$ [Myrs]",fontsize=FONTSIZE)
    axd["middle"].grid(True, which='both', alpha=GRID_ALPHA)


    ratio0 = np.array([np.mean(nlos0/ncrs) for (nlos0,ncrs) in zip(data2a2["Nlos0"],data2a2["Ncrs"])]) 
    ratio1 = np.array([np.median(nlos1/ncrs) for (nlos1,ncrs) in zip(data2a2["Nlos0"],data2a2["Ncrs"])])
    ratio2 = np.array([np.std(nlos0/ncrs) for (nlos0,ncrs) in zip(data2a2["Nlos0"],data2a2["Ncrs"])])

    texto = r"$r^{non-ideal}_{cloud} = 0.5 \ \rm{pc}$"
    axd["right"].text(data2a2['t'][0], 1.70, texto, fontsize=12, color="black", ha="left", va="bottom")
    axd["right"].plot(data2a2['t'], ratio0, 'o-', label="Mean ratio")
    axd["right"].plot(data2a2['t'], ratio1, '.-', label="Median ratio")
    axd["right"].plot(data2a2['t'], ratio0 + ratio2, '--', color=COLORS[1] )
    axd["right"].plot(data2a2['t'], ratio0 - ratio2, '--', color=COLORS[1])
    axd["right"].set_xlabel("$t$ [Myrs]",fontsize=FONTSIZE)
    axd["right"].tick_params(labelleft=False)
    axd["right"].grid(True, which='both', alpha=GRID_ALPHA)
    fig.suptitle(title)
    plt.savefig( "./series/" + 'ratiosa012.png', dpi=300)
    plt.close(fig)
    ...
    axd["right"].grid(True, which='both',  alpha=GRID_ALPHA)
    fig.suptitle(title)
    plt.savefig( "./series/" + 'ratiosi34.png', dpi=300)
    plt.close(fig)
    ...
    title = r"Ratio $N^{non-ideal}_{los}/N^{non-ideal}_{crs}$"

    ratio0 = np.array([np.mean(nlos0/ncrs) for (nlos0,ncrs) in zip(data4a3["Nlos0"],data4a3["Ncrs"])]) 
    ratio1 = np.array([np.median(nlos1/ncrs) for (nlos1,ncrs) in zip(data4a3["Nlos0"],data4a3["Ncrs"])])
    ratio2 = np.array([np.std(nlos0/ncrs) for (nlos0,ncrs) in zip(data4a3["Nlos0"],data4a3["Ncrs"])])

    #axd["left"].plot(data2i0['t'], np.ones_like(data2i0['t']), '--', color="black")
    axd["left"].plot(data4a3['t'], ratio0, 'o-', label=r"Mean $N_{los}/N_{crs}$")
    axd["left"].plot(data4a3['t'], ratio1, '.-', label=r"Median $N_{los}/N_{crs}$")
    axd["left"].plot(data4a3['t'], ratio0 + ratio2, '--', color=COLORS[1], label="$\sigma$-band")
    axd["left"].plot(data4a3['t'], ratio0 - ratio2, '--', color=COLORS[1])
    axd["left"].set_ylabel(r"$N_{los}/N_{crs}$",fontsize=FONTSIZE)
    axd["left"].set_ylim(-0.0,2.5)
    axd["left"].set_xlabel("$t$ [Myrs]",fontsize=FONTSIZE)
    axd["left"].grid(True, which='both', alpha=GRID_ALPHA)
    ...
    ratio0 = np.array([np.mean(nlos0/ncrs) for (nlos0,ncrs) in zip(data6a4["Nlos0"],data6a4["Ncrs"])]) 
    ratio1 = np.array([np.median(nlos1/ncrs) for (nlos1,ncrs) in zip(data6a4["Nlos0"],data6a4["Ncrs"])])
    ratio2 = np.array([np.std(nlos0/ncrs) for (nlos0,ncrs) in zip(data6a4["Nlos0"],data6a4["Ncrs"])])

    #axd["right"].plot(data2i2['t'], np.ones_like(data2i2['t']), '--', color="black")
    axd["right"].plot(data6a4['t'], ratio0, 'o-', label="Mean ratio")
    axd["right"].plot(data6a4['t'], ratio1, '.-', label="Median ratio")
    axd["right"].plot(data6a4['t'], ratio0 + ratio2, '--', color=COLORS[1] )
    axd["right"].plot(data6a4['t'], ratio0 - ratio2, '--', color=COLORS[1])
    axd["right"].set_ylim(-0.0,2.5)
    axd["right"].set_xlabel("$t$ [Myrs]",fontsize=FONTSIZE)
    axd["right"].tick_params(labelleft=False)
    axd["right"].grid(True, which='both',  alpha=GRID_ALPHA)
    fig.suptitle(title)
    plt.savefig( "./series/" + 'ratiosa34.png', dpi=300)
    plt.close(fig)


if True:
    five_sample_mask = np.zeros(len(data2i0["t"]), dtype=bool)
    five_sample_mask[:] = True
    ...
    for __, data in enumerate(datas): 
        ...
        ax.set_ylabel(r"$\mathscr{R}$ [adimensional]", fontsize=FONTSIZE)
        ax.grid(True, which='both',alpha=GRID_ALPHA)
        ...
        for ptile in percentiles[::-1]:
            ...
            if data['t'].shape[0] < 4:
                midpnt = data['t'].shape[0] // 2
                ax.text(data['t'][midpnt], f_ptile_up[midpnt], f"P{100-ptile}",
                        fontsize=6, color="black", ha="left", va="bottom")
                ax.text(data['t'][midpnt], f_ptile_down[midpnt], f"P{ptile}",
                        fontsize=6, color="black", ha="left", va="bottom")        
            else:
        ...
        if np.sum(at_colapse) == 0:
            print("Mask is empty, ", np.sum(at_colapse))
            continue
        ...
        ax1.set_ylabel(r"$\mathscr{R}$ [adimensional]", fontsize=FONTSIZE)
        ...
        sm.set_array([])
        cbar = fig.colorbar(sm, ax=ax)
        cbar.set_label(rf'$t - {x1}$ Myrs [kyrs]', fontsize=FONTSIZE)
...
if True:
    five_sample_mask = np.zeros(len(data2a0["t"]), dtype=bool)
    five_sample_mask[:] = True

    t_min = np.min([data2a0["t"].min(), data2a1["t"].min(), data2a2["t"].min()])
    t_max = np.max([data2a0["t"].max(), data2a1["t"].max(), data2a2["t"].max()])
    ...
    for j, data in enumerate([data2a0, data2a1, data2a2]):
        time   = data["t"][five_sample_mask]  
        surf   = data["surf"][five_sample_mask]  
        factu  = data["factu"][five_sample_mask]
        densit = data["n"][five_sample_mask]

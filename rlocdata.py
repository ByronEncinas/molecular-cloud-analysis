...
#    datas += [data6a4, data2a0, data2a1, data2a2, data4a3]
#elif sys.argv[1] == "-e":
datas = []
for f in files:
    ID = f.split('.')[-2][n:]
...
plt.savefig(f'./paper/e-filteredout.png', dpi=150, bbox_inches='tight')
plt.close(fig)
...
for _, data_j in enumerate(datas): 
    ...
    for j in range(total_snaps-1,0,-total_snaps//5):
        ...
        ax0.set_title(f'$t$ = {round(_time, 5)}  Myrs\nPoints: ' + f'$n_c > 10^2$ ' + r'$\rm{cm}^{-3}$', fontsize = FONTSIZE)
        ...
        plt.savefig(f'./series/{_id}emap{j}.png')
        plt.close(fig)
    ...
    ratio0 = np.array([np.mean(nlos0/ncrs) for (nlos0,ncrs) in zip(data_j["Nlos0"],data_j["Ncrs"])]) 
    ratio1 = np.array([np.median(nlos1/ncrs) for (nlos1,ncrs) in zip(data_j["Nlos0"],data_j["Ncrs"])])
    ratio2 = np.array([np.std(nlos0/ncrs) for (nlos0,ncrs) in zip(data_j["Nlos0"],data_j["Ncrs"])])

...
def color_voronoi_plot(VP, D, var=''):
...
def voronoi_plot(VP, out_radius):
...
def match_files_to_data(__input_case__):
    ...
    if 'ideal' in __input_case__:
        ...
        file_hdf5 = np.array(glob.glob(f'../m-c-stats/arepo_data/{subdirectory}/*.hdf5'))
...
rlocs = [__rloc__, __rloc__/10.0, __rloc__/100.0, __rloc__/1_000.0]
...
if __name__=='__main__':
    ...
    for each, filename in enumerate(file_hdf5):
        center   = clst[each, :]
        _time    = tlst[each]
        #print(filename.split('.')[-2][-3:])
        snap = int(filename.split('.')[-2][-3:])
        if snap < 400:
            continue
        ...
        for __ith_rloc__ in rlocs[::-1]:
            CoreDensity = dlst[each]
            X,Y,Z  = VoronoiPos[:,0], VoronoiPos[:,1], VoronoiPos[:,2]

            mask= np.logical_and((X*X + Y*Y+ Z*Z < __ith_rloc__*__ith_rloc__),  
                    np.logical_and((Z < __ith_rloc__*0.01),(Z > -__ith_rloc__*0.01)))

            #mask1 = Density > 1.0e+0
            #mask = np.logical_and(mask1, mask)
            print(snap, __ith_rloc__, np.sum(mask))
            densities = Density[mask]  # assuming Density array matches VP shape
            vor = Voronoi(VoronoiPos[mask, :2])
            frame_radius = __ith_rloc__/2
            polygons = []
            colors = []
            ...
            ax.set_ylabel('y')
            fig.tight_layout()
            plt.savefig(f'./vor/vor{np.log10(__ith_rloc__)}-{snap}.png', dpi=150)
            ...
            total_volume = Volume[mask_sphere].shape[0]
            print(total_cells)
            ...
            fig.suptitle(f"Case {subdirectory} - Snapshot: {snap}\n$r_loc = {__ith_rloc__}$", fontsize=13)
            ...
            for ax, fractions, title, color in zip(
                ...
                ax.set_xlabel("Cloud Density")
                ax.set_ylabel("Volume fraction")
                ax.set_title(title)
                ax.grid(True, which="both", linestyle="--", alpha=0.5)

            plt.tight_layout()
            plt.savefig(f"./densfrac{np.log10(__ith_rloc__)}{_id_[0]}{snap}.png", dpi=150)
            plt.close(fig)
